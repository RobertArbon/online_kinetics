...
class TestHedgeOptimizer:
    ...
    def test_learning_rate_influence_on_weight_changes(self, base_config, sample_batch):
        ...
        assert (
            total_change_high > total_change_low
    ...
    def test_beta_influence_on_layer_weights(self, base_config, sample_batch):
        ...
        initial_layer_weights_high = (
            estimator_high_beta.model.layer_weights.data.clone()
        )
        ...
        assert (
            change_low_beta > change_high_beta
    ...
    def test_gamma_minimum_weight_constraint(self, base_config, sample_batch):
        ...
        # loss1, loss2, beta, gamma, alpha1, alpha2 (these are calculated in excel)
        cases = [
            (0.1, 10, 0.9, 0.5, 0.6643212250, 0.3356787750),
        ...
        for l1, l2, beta, gamma, alpha1, alpha2 in cases:
            ...
            def mock_update_with_extreme_losses(layer_losses):
                mock_losses = [torch.tensor(l1), torch.tensor(l2)]
            ...
            assert torch.allclose(
                final_weights, torch.tensor([alpha1, alpha2])
            ), f"{final_weights}"
    ...
    def test_gradient_accumulation_correctness(self, base_config, sample_batch):
        ...
        assert (
            params_changed
        ), "At least some parameters should have changed after optimization"
    ...
    def test_different_layer_configurations(self, base_config, sample_batch):
        ...
        for n_layers in [1, 3, 4]:
            ...
            for weight in estimator.model.layer_weights:
                assert (
                    abs(weight.item() - expected_initial_weight) < 1e-6
    ...
    def test_deterministic_behavior(self, base_config, sample_batch):
        ...
        assert torch.allclose(
            estimator1.model.layer_weights, estimator2.model.layer_weights, atol=1e-6
        ), "Optimizer should produce deterministic results"

        # Check that model parameters are identical
        for (name1, param1), (name2, param2) in zip(
            estimator1.model.named_parameters(), estimator2.model.named_parameters()
        ):
            assert name1 == name2
            assert torch.allclose(
    ...
    def test_zero_learning_rate_no_change(self, base_config, sample_batch):
        ...
        for name, param in estimator.model.named_parameters():
            if (
                "layer_weights" not in name
            ):  # Skip layer weights as they're updated by hedge algorithm
                assert torch.allclose(
                    param.data, initial_params[name], atol=1e-6
    ...
    def test_layer_weight_updates_direction(self, base_config, sample_batch):
        ...
        assert torch.all(
            final_layer_weights >= 0
        ), "All layer weights should be non-negative"
        assert torch.all(final_layer_weights <= 1), "All layer weights should be <= 1"
        assert (
            abs(torch.sum(final_layer_weights).item() - 1.0) < 1e-6
        ...
        assert (
            total_weight_change > 1e-8
        ), "Layer weights should have changed during optimization"
