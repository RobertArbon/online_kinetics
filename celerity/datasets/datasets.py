...
def alanine_dipeptide_trajectories(
    n_trajectories: int = 1, n_frames: int = 10, data_dir: str = None
    ...
    logger.info(
        f"Fetching {n_trajectories} alanine dipeptide trajectories with "
        f"{p_n_frames} frames"
    )
    trajs = []
    for i in range(n_trajectories):
        try:
            ...
            logger.exception(
                f"Couldn't load alanine-dipeptide trajectory", exc_info=True
            )
            return trajs
    return trajs


def alanine_dipeptide_features(
    feature: str,
    ...
    for i in range(n_trajectories):
        try:
            ...
            logger.exception(
                f"Couldn't load alanine-dipeptide feature array", exc_info=True
            )
            return trajs
